...
def read_X(path, quantity, no=1):

    if quantity=='X_avg':
        f_X_iter = sorted(glob(path + "/*_phonon_iteration_X_avg.dat"))
    elif quantity=='X0':
        f_X_iter = sorted(glob(path + "/*_phonon_iteration_X0.dat"))
    
    #f_X_iter = sorted(glob(path + "/*_density_u.dat"))
    #f_X_iter = sorted(glob(path + "/*_phonon_X_avg.dat"))

    print 'l',len(f_X_iter)
    ...
    for i, f in enumerate(f_X_iter):
        mydata = np.fromfile(f)
        if len(mydata)>=10000:

            if len(mydata)==33333: #beta = index 4
                X_iter.append(mydata[:30000])
                num_sample = 30000
            elif len(mydata)==66666: #beta = index 6
                X_iter.append(mydata[:60000])
                num_sample = 60000
            elif len(mydata)==83333: #beta = index 7
                X_iter.append(mydata[:80000])
                num_sample = 80000
            else:
                X_iter.append(mydata)
                num_sample = len(mydata)

        #print 'A ',np.shape(np.fromfile(f))

    

    X_iter = np.array(X_iter)

    print ' shape ..... ',np.shape(X_iter)

    print("number of bins:", nbin)
    print("number of measurements per bin:", X_iter.shape[1])
    ac_len = autocorr_length(X_iter)/num_sample
...
def main(argv):
    ...
    for path in folders:
        print("reading path:")
        print(path)
        ac_lengths.append(read_X(path))

    print 'ac_length = [',
    for i in range(len(ac_lengths)):
        print ac_lengths[i],
        if i < len(ac_lengths)-1:
            print ', ',
    print ']'
'''
